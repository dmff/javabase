 ...
 *         具体实现上使用了Map<String,Boolean>，键值对分别是链接和是否被遍历标志。程序中使用了两个Map集合，分别是：oldMap和newMap，初始的链接在oldMap中,
 *         然后对oldMap里面的标志为false的链接发起请求，解析页面，用正则取出<a>标签下的链接，如果这个链接未在oldMap和newMap中，则说明这是一条新的链接，
 *         同时要是这条链接是我们需要获取的目标网站的链接的话，我们就将这条链接放入newMap中，一直解析下去，等这个页面解析完成，把oldMap中当前页面的那条链接的值设为true，
 *         表示已经遍历过了。最后是当整个oldMap未遍历过的链接都遍历结束后，如果发现newMap不为空，则说明这一次循环有新的链接产生，因此将这些新的链接加入oldMap中，继续递归遍历，
...
public class WebCrawlerDemo {
	...
	public void myPrint(String baseUrl) {
		...
		Pattern p = Pattern.compile("(http?://)?[^/\\s]*"); // 比如：http://www.zifangsky.cn
